    ...
    public class Skill_Lightning_1 : Skill
    {
        ...
        void makeLightning()
        {
            for(int i = 0;i<SelectEnemies.Count;i++){
                if(SelectEnemies[i].transform.position.x >= ResolutionManager.ResolutionX * 0.5f) continue;
                if(SelectEnemies[i].transform.position.x <= ResolutionManager.ResolutionX * 0.5f * -1) continue;
                if(SelectEnemies[i].transform.position.y >= ResolutionManager.ResolutionY * 0.5f) continue;
                if(SelectEnemies[i].transform.position.y <= ResolutionManager.ResolutionY * 0.5f * -1) continue;
